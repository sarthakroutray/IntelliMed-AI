      ...
      googleLogin(response.credential, activeTab)
        ...
        .catch((err) => {
          ...
          if (err.response?.status === 403 && errorMessage.includes('registered as')) {
            const oppositeRole = activeTab === 'patient' ? 'doctor' : 'patient';
            setError(`${errorMessage} Already have an account? Try signing in as a ${oppositeRole} or go to the login page.`);
          } else {
          ...
          <div className="p-8 sm:p-10 flex flex-col items-center">
            ...
            <div className="text-center w-full mb-6">
              ...
              <p className="text-[#616f89] dark:text-gray-400 text-sm font-normal leading-normal px-4">
                Join IntelliMed-AI and take control of your health journey
              </p>
            </div>
            ...
            {/* Error Message */}
            {error && (
              <div className="w-full mb-4 p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-sm">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  required
                  disabled={loading}
                  className="w-full px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-primary focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                />
              </div>
              ...
              {activeTab === 'doctor' && (
                <div>
                  <label htmlFor="doctorAccessCode" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                    Doctor Access Code
                  </label>
                  <input
                    type="text"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full h-11 bg-primary hover:bg-blue-700 text-white font-bold rounded-lg shadow-md shadow-primary/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mt-2"
              >
                {loading ? (
                  <>
